...
def make_predictions(comp_list, chargefrom_list, chargeto_list, lattice_list, bandgap_list, is_b_latt_list):

    # Process data
    df_test = process_data(comp_list)

    # Add other features to df_test
    df_test['charge_from'] = chargefrom_list
    ...
    pred_dict = {'Compositions': comp_list,
                 'Predicted defect level (eV)': preds,
                 'Ebar defect level (eV)': ebars}
